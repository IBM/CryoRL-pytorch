...
# Create a dataframe
df = pd.DataFrame({'#lCTFs': np.r_[greedy_GT, greedy_Res18, greedy_Res50, cryoRL_GT, cryoRL_Res18, cryoRL_Res50],
                   'Time': np.r_[np.repeat(period, n), np.repeat(period, n), np.repeat(period, n), np.repeat(period, n), np.repeat(period, n), np.repeat(period, n)],
                   'Method': np.repeat(['greedy_GT', 'greedy_R18', 'greedy_R50', 'cryoRL_GT',  'cryoRL_R18', 'cryoRL_R50'], len(period) * n),
                   'Subject': np.r_[np.tile(np.arange(n), 4),
                                    np.tile(np.arange(n, n + n), 4),
                                    np.tile(np.arange(2*n, 3*n), 4),
                                    np.tile(np.arange(3*n, 4*n), 4),
                                    np.tile(np.arange(4*n, 5*n), 4),
                                    np.tile(np.arange(5*n, 6*n), 4)]})

sns.set()
plot = sns.pointplot(data=df, x='Time', y='#lCTFs', hue='Method', dodge=True, markers=['o', 's', 'x', 'p', 'v', '*'],
	    capsize=.1, errwidth=1, palette='colorblind')
# plot.errorbar([])
plot.legend_.set_title(None)
fig = plot.get_figure()
fig.savefig("main-results-8bits-new.pdf")
