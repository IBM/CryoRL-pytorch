...
'''
def cryoEM_loader(names, CTFs):  # CryoEM_data from the csv
    grids = set()
    squares = set()
    patches = set()
    for j in CTFs.keys():
    ...
    for g_th, (grid_name, grid) in enumerate(gr_sq_hl_en.items()):
        ...
        for i_th, (square_name, square) in enumerate(grid.items()):
            ...
            for j_th, (patch_name, patch) in enumerate(square.items()):
                holeList = list()
                for k_th, (hole_name, hole_CTF) in enumerate(patch.items()):
                    if hole_CTF > CryoEMConfig.MAX_CTF_VALUE:
                        hole_CTF = CryoEMConfig.MAX_CTF_VALUE
                    hole_k = CryoEMHole(hole_name, k_th, j_th, ctf=hole_CTF)
                    holeList.append(hole_k)
                    index_list.append((g_th, i_th, j_th, k_th))
                patch_j = CryoEMPatch(patch_name, j_th, i_th, holeList=holeList)
                patchList.append(patch_j)
            square_i = CryoEMSquare(square_name, i_th, j_th, patchList=patchList)
            squareList.append(square_i)
        grid_g = CryoEMGrid(grid_name, g_th, squareList=squareList)
        cryoEM_data.append(grid_g)
